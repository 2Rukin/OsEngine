...
### 4.12. Независимые рабочие окна и порядок перекрытия

Окна приложения не закрепляются принудительно поверх других программ через
`Topmost="True"`. Рабочее окно, с которым пользователь должен работать
параллельно основному, открывай через `Show()` без WPF `Owner`: владелец
удерживает дочернее окно перед собой даже при `Topmost=false`.
Храни ссылку на окно, повторное открытие восстанавливает его из свёрнутого
состояния и активирует, а закрытие основного окна явно закрывает связанные
рабочие окна и освобождает подписки. `ShowDialog()` сохраняется для операций,
которым нужен ответ пользователя перед продолжением, например подтверждения
или редактирования настроек.
