...
Документ сохраняет формулы и критерии исходного задания; операции экрана —
в [runbook](RESEARCH_MVP_RUNBOOK.md#48-исследование-cloud--отдельный-explorer-v2),
...
Explorer открывается в отдельном немодальном owned Window. Повторный выбор
активирует уже открытое окно; после его закрытия следующий выбор создаёт новое.
...
sequenceDiagram
    ...
    alt Explorer уже открыт
        Tab->>Explorer: Активировать
    else Explorer закрыт
        Tab->>Explorer: Создать и показать немодально
    end
    Explorer-->>Main: Закрытие освобождает owned workbench
```
