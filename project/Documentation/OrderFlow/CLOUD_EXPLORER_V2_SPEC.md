...
Доступность основного окна для ввода и свободный порядок окон остаются
требованием, не подтверждённым ручной проверкой владельца; открытый дефект
учтён в [техническом долге](TECHNICAL_DEBT.md).
...
sequenceDiagram
    ...
    Note over Main,Explorer: Свободный ввод и порядок окон пока не подтверждены
