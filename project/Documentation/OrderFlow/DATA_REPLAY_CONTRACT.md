...
Правила непрерывных окон и последовательность §3 ниже относятся к прежнему
Delta/Cloud workbench. Отдельный opt-in Explorer V2 использует тот же reader,
ручной шаг и inclusive даты, но имеет явные source-date cutoff, собственный
порядок модулей и независимые hashes по
[ORDER-FLOW-CLOUD-EXPLORER-V2-001](CLOUD_EXPLORER_V2_SPEC.md#11-причинный-реплей).
Он не меняет описанный ниже legacy replay. Секундная точность источника не
доказывает межрыночный порядок или реальную паузу внутри одной секунды.
