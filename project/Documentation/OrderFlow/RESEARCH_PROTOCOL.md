...
Отдельный реализованный Explorer V2 добавляет prefix-causal Cloud/episode
trigger, подтверждённые swings и terminal structural watches. Его сравнение
`VolumeArmed`/`Control`, исключённые/неизвестные случаи и future labels заданы
только в [контракте V2](CLOUD_EXPLORER_V2_SPEC.md#9-сопоставление-с-будущей-реакцией).
Это дополнительная двухчастная exploratory оценка, не полная квалификация
policy или замена target protocol ниже; старое исследование реакций не изменено.
