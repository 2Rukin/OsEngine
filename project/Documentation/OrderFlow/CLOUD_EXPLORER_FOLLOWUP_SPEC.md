...
**Статус:** TARGET SPECIFICATION — функции и исправления ниже не объявлены реализованными.
...
`3/9/18` минут. Поиск комбинаций и каталог найденных сценариев **пока
отсутствуют**. Сохранить все действующие bundles, их схемы, legacy вкладки,
...
В `ExplorerRunSpec.Validate` сейчас общая английская ошибка объединяет путь,
даты, профили и лимиты. Ввести адресную проверку **до запуска worker**:
...
Сейчас сводка записывается в многострочный `TextBoxSummary` и после установки
...
Сейчас `ExplorerChart` строит общий горизонтальный диапазон по Cloud, барам,
пивотам и наблюдениям, тогда как `LoadPage` загружает свечи по диапазону
только текущей страницы Cloud. Поэтому метки за пределами свечей способны
растянуть время. Колесо меняет горизонтальный масштаб; отдельного изменения
шкалы цены нет. Ввести единый **явно показанный временной интервал**: для
