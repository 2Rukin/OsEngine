...
Свободное переключение и ввод при открытом Cloud пока не подтверждены:
сохраняющаяся проблема отложена в
[TD-CLOUD-WINDOW-001](TECHNICAL_DEBT.md#td-cloud-window-001--cloud-перекрывает-order-flow-и-блокирует-ввод).
До её закрытия задавайте входные параметры перед открытием Cloud.
...
Если нужно поменять файл, папку, шаг цены или даты, эти поля находятся в основном
окне Order Flow. С учётом открытого техдолга может потребоваться закрыть Cloud,
изменить поля и открыть его заново. Закрытие отменяет текущую работу Explorer;
опубликованные файлы результата не удаляются.
