...
| `ORDER-FLOW-TECH-DEBT-001` | `project/Documentation/OrderFlow/TECHNICAL_DEBT.md` | CURRENT ISSUE REGISTER — OPEN FOLLOWUP ITEMS | Наблюдения владельца о Cloud Explorer, границы подтверждения, открытые проблемы и закрытая оконная запись |
...
| `ORDER-FLOW-CLOUD-EXPLORER-FOLLOWUP-001` | `project/Documentation/OrderFlow/CLOUD_EXPLORER_FOLLOWUP_SPEC.md` | TARGET SPECIFICATION — NOT IMPLEMENTED | Приёмка интерфейсных исправлений и автоматического поиска предвестников внутридневного движения с недельным контекстом |
